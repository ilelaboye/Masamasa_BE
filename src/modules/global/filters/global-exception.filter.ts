...
export class GlobalExceptionFilter implements ExceptionFilter {
  ...
  constructor(private httpAdapterHost: HttpAdapterHost) {}
    ...
    this.winstonLogger.error(`Exception: ${exception}\nStatus: ${httpStatus}\nERROR: ${exception}`);

    const msg = exception.message || 'Bad request',
      statusCode = msg.includes('typeorm') ? HttpStatus.INTERNAL_SERVER_ERROR : httpStatus;
    ...
    httpAdapter.reply(
      ctx.getResponse(),
      {
        ...
        stackTrace: errorEnvironment.includes(appConfig.ENV) && appConfig.DEBUG === 'true' ? exception : undefined,
      },
      statusCode,
    );
  }
