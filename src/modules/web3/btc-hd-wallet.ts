...
export class BtcHDWallet {
  ...
  async sweepBTC(
    ...
    const { data: utxos } = await axios.get(
      `${baseUrl}/address/${childAddress}/utxo`,
    );
  ...
  async withdrawBTC(
    ...
    const masterIdx = 0;
    const masterNode = this.root.derivePath(
      `m/84'/${this.network === bitcoin.networks.testnet ? "1" : "0"}'/0'/0/${masterIdx}`,
    );
    const masterAddr = this.generateAddress(masterIdx);

    if (masterAddr !== masterAddress) {
      console.warn(
        "Provided master address does not match derived master address (index 0)",
      );
    }
    ...
    // 1. Get UTXOs
    const { data: utxos } = await axios.get(
      `${baseUrl}/address/${masterAddr}/utxo`,
    );
    ...
    if (totalInput < sendAmountSat + fee) {
      throw new Error(
        `Insufficient balance to cover amount + fee (${Number(fee) / 1e8} BTC)`,
      );
    }
