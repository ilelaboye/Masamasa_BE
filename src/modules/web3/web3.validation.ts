...
export const CreateDisposableWalletValidation = Joi.object({
  network: Joi.string()
    ...
    .messages({
      "any.only":
        "Network must be one of: BASE, ETH, BSC, POLYGON, SOLANA, TRON, BITCOIN, CARDANO, RIPPLE, DOGE",
      "any.required": "Network is required",
    }),

  expectedAmount: Joi.number().positive().optional().messages({
    "number.positive": "Expected amount must be positive",
  }),

  tokenSymbol: Joi.string().uppercase().optional().messages({
...
export const CheckDisposableWalletValidation = Joi.object({
  ...
  destinationTag: Joi.number().integer().optional(),
});
...
export const SweepDisposableWalletValidation = Joi.object({
  ...
  tokenSymbol: Joi.string().uppercase().optional(),
