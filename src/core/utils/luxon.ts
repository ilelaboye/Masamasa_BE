...
export const formateMailDate = (dateTime: string | Date) => {
  const dt = DateTime.fromISO(new Date(dateTime).toISOString()).toLocaleString(
    DateTime.DATE_FULL,
  );
  return dt;
};
...
export const getDateTime = (time = 0) =>
  DateTime.local().plus({ minutes: time }).toISO();
...
export const dateHasExpired = (createdAt: string | Date) =>
  timeIsAfter(createdAt, 0);

export const startOfDay = (date: Date) =>
  DateTime.fromJSDate(date).startOf("day").toISODate();
export const endOfDay = (date: Date) =>
  DateTime.fromJSDate(date).endOf("day").toISO();

export const startOfMonth = (date: Date) =>
  DateTime.fromJSDate(date).startOf("month").toISODate();

export const endOfMonth = (date: Date) =>
  DateTime.fromJSDate(date).endOf("month").toISODate();

export const startOfYear = (date: Date) =>
  DateTime.fromJSDate(date).startOf("year").toISODate();

export const endOfYear = (date: Date) =>
  DateTime.fromJSDate(date).endOf("year").toISODate();
...
export const formatToLuxon2 = (date: string) =>
